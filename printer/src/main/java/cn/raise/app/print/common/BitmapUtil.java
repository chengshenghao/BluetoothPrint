...
public class BitmapUtil {
     ...
     * @return
     */
    public static Bitmap createQRBitmap(String str, int width, int height, int res) {
        try {
            ...
            Bitmap bitmap = Bitmap.createBitmap(width + 2 * res, height + 2 * res, Bitmap.Config.ARGB_8888);
            //设置画布
            Canvas canvas = new Canvas(bitmap);
            //画布背景白色
            canvas.drawColor(Color.WHITE);
            bitmap.setPixels(pixels, 0, width, res, res, width, height);
            saveBitmapToSDCard(bitmap);
            Log.d(TAG, "createQRBitmap: " + bitmap.getWidth() + ":" + bitmap.getHeight());
            return bitmap;
        } catch (Exception e) {
            e.printStackTrace();
            return null;
