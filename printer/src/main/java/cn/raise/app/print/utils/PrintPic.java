...
public class PrintPic {
     ...
     *
     * @param bitmap
     * @param paperWidth 打印纸的像素宽度
     */
    public void init(Bitmap bitmap, int paperWidth) {
        if (null != bitmap) {
            initCanvas(paperWidth, bitmap.getHeight());
        }
        if (null == paint) {
            initPaint();
        }
        if (null != bitmap) {
            drawImage((paperWidth - bitmap.getWidth()) / 2, 0, bitmap);
        }
    }
     ...
     * @param bitmapHeight
     */
    public void initCanvas(int paperWidth, int bitmapHeight) {
        int h = 10 * bitmapHeight;

        this.bm = Bitmap.createBitmap(paperWidth, h, Bitmap.Config.RGB_565);
        this.canvas = new Canvas(this.bm);
    ...
    public byte[] printBitmap(Bitmap bitmap, int width, int height) {
        Bitmap bm1 = bitmap;
        Bitmap bm = Bitmap.createBitmap(width, height, Bitmap.Config.RGB_565);
        Canvas canvas = new Canvas(bm);
        canvas.drawColor(-1);
        canvas.drawBitmap(bm1, 0, 0, null);
        Bitmap nbm = Bitmap.createBitmap(bm, 0, 0, width, height);
        byte[] imgbuf = new byte[width / 8 * height + 4];
        // 打印光栅位图的指令
        imgbuf[0] = 29;// 十六进制0x1D
        imgbuf[1] = 42;// 十六进制0x76
        imgbuf[2] = (byte) (width / 8);// 30
        imgbuf[3] = (byte) (height / 8);// 位图模式 0,1,2,3
        int s = 4;
        for (int i = 0; i < width; i++) {// 循环位图的高度
            for (int k = 0; k < height / 8; k++) {// 循环位图的宽度
                int c0 = nbm.getPixel(i, k * 8);// 返回指定坐标的颜色
                int p0;
                if (c0 == -1)// 判断颜色是不是白色
                    p0 = 0;// 0,不打印该点
                else {
